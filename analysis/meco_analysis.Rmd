...
meco_agg_regressions = meco_sacc_df %>%
  ...
  group_by(text_id, sent_id, curr_ia_idx, next_ia_idx, lang) %>%
    summarise(n_regressions = n(), n_subjs = length(unique(subj)), prop_subj = n_subjs / total_subj ) %>%
  ungroup() %>%
  rename(
...
meco_df %>%
  ...
  mutate(is_max = trigger_idx == max_idx) %>%
  group_by(is_max) %>%
    summarise(m_regress = mean(n_regressions),
              m_dist = mean(dist)) %>%
  ungroup()
...
meco_df %>%
  arrange(lang, text_id, sent_id, trigger_idx)
...
plot_df %>%
  ...
  ggplot(aes(x = tag, y=m, color = theory)) +
    ...
    geom_text(aes(label = sig, color = theory), y = 0.13, size = 3, angle = 0) +
    facet_wrap(.~lang) +
    ylab("Delta LogLik") +
    labs(color="") +
...
for (l in langs){
    ...
    dll_npmi_comp = rbind(dll_npmi_comp, data.frame(lang = l, m=mean(dll_diff), pval = as.numeric(ptest$p.value)))
}

write.csv(dll_npmi_comp, "./images/meco_dll_npmi_comp.csv")
