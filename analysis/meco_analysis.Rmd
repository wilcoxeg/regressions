...
combined_df %>%
  ...
  group_by(lang, pmi_sign) %>%
    summarise(n = n()) %>%
  ungroup()

combined_df %>%
  drop_na() %>%
  filter(trigger_idx > 6) %>%
  filter(target_idx > 6) %>%
...
m_pmi = combined_df %>%
  filter(lang == "it") %>%
  glm("n_regressions ~ dist + target_len + target_freq + target_surp + trigger_len + trigger_freq + trigger_surp + npmi + ppmi", data = ., family = poisson())
summary(m_pmi)
...
for (l in langs){
    ...
    dll_diff = dll_diff[!is.na(dll_diff)]
    ptest = perm.test(dll_diff, num.sim = 500)
    dll_epmi_ens_comp = rbind(dll_epmi_ens_comp, data.frame(lang = l, epmi_ens_m=mean(dll_diff), epmi_ens_pval = as.numeric(ptest$p.value)))
}
dll_epmi_ens_comp 
```
...
plot_df %>%
  ...
  mutate(sig = pval_func(pval),
         pmi_ens_pval = pval_func(pmi_ens_pval),
         epmi_ens_pval = pval_func(epmi_ens_pval),
  ...
  ggplot(aes(x = tag, y=m, color = theory, fill = theory)) +
    ...
    geom_segment(aes(y = y_max + 1 * y_step, yend = y_max + 13 * y_step), size =0.5, x = 1.95, xend = 5.05, color="gray", alpha=0.4) +
    geom_text(aes(label = npi_pval, y = y_max + y_step * 1.5, x = 3.5), size = 3, angle = 0, color = "gray", alpha=0.2) +
