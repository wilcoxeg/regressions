...
plot_df %>%
  ...
  ggplot(aes(x = tag, y=m, color = theory)) +
    ...
    geom_text(aes(label = sig, color = theory, y=case_when(corpus=="Dundee" ~ 0.0015, corpus =="Provo" ~ 0.1, corpus == "UCL"~0.031)), size = 3) +
    # This is just some dummy whitespace to make the facet scales look nice.
    geom_text(aes(label = "", y=case_when(corpus=="Dundee" ~ 0.00155, corpus =="Provo" ~ 0.11, corpus == "UCL"~0.033)), size = 1) +
