...
combined_df = rbind(combined_df, dundee_df)%>%
  ...
  # Filter out sentence-final words because of wrap-up effects
  mutate(dist = trigger_idx-target_idx) %>%
  group_by(model, corpus, text_id, sent_id) %>%
...
plot_df %>%
  ...
  ggplot(aes(x = tag, y=m, color = theory)) +
    ...
    geom_text(aes(label = sig, color = theory, y=case_when(corpus=="Dundee" ~ 0.0011, corpus =="Provo" ~ 0.09, corpus == "UCL"~0.015)), size = 3) +
    # This is just some dummy whitespace to make the facet scales look nice.
    geom_text(aes(label = "", y=case_when(corpus=="Dundee" ~ 0.00115, corpus =="Provo" ~ 0.095, corpus == "UCL"~0.0155)), size = 1) +
...
write.csv(dll_npmi_comp, "./images/en_dll_npmi_comp.csv")
