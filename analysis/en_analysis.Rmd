...
m_pmi = combined_df %>%
  filter(model == "BERT-maskRC", corpus == "ucl") %>%
  glm("n_regressions ~ dist + target_len + target_freq + target_surp + trigger_len + trigger_freq + trigger_surp + pmi + e_npmi", data = ., family = poisson())
summary(m_pmi)
...
plot_df %>%
  ...
  ggplot(aes(x = tag, y=m, color = theory)) +
    ...
    geom_text(aes(label = sig, color = theory, y=case_when(corpus=="Dundee" ~ 0.0015, corpus =="Provo" ~ 0.11, corpus == "UCL"~0.035)), size = 3) +
    # This is just some dummy whitespace to make the facet scales look nice.
    geom_text(aes(label = "", y=case_when(corpus=="Dundee" ~ 0.0018, corpus =="Provo" ~ 0.115, corpus == "UCL"~0.038)), size = 1) +
