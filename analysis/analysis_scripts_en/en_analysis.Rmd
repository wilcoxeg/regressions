...
# To run the basic analysis use:
corpora = c("provo", "ucl", "dundee")
windows = c("w0")
models = c("BERT-allRC", "BERT-maskRC", "BERT-dropRC")

# To run the window analysis use:
#corpora = c("provo", "ucl")
#windows = c("w0", "w1_mean", "w1_max")
#model = c("BERT-allRC", "BERT-maskRC", "BERT-dropRC")

# Placing large files not in github directory
data_path = "../../../../research/regressions/ancillary_data/"

```

```{r}

combined_df = read_rds(paste0(data_path, "combined_df_raw.csv"))
...
saveRDS(m_provo_zip, "../fits/m_provo_zip.rds")
summary(m_provo_zip$count$coefficients)

m_ucl_zip = combined_df %>% filter(corpus == "ucl") %>%
  filter(window == "w0") %>%
  mutate(window = as.factor(window), model = as.factor(model)) %>%
  glmmTMB(formula=as.formula(formula), data=., family=poisson, ziformula = as.formula(formula))
saveRDS(m_ucl_zip, "../fits/en_ucl_zip.rds")
print(summary(m_ucl_zip))

m_dundee_zip = combined_df %>% filter(corpus == "dundee") %>%
  filter(window == "w0") %>%
  mutate(model = as.factor(model)) %>%
  glmmTMB(formula=as.formula(formula), data=., family=poisson, ziformula = as.formula(formula))
saveRDS(m_dundee_zip, "../fits/en_dundee_zip.rds")
print(summary(m_dundee_zip))
...
model_cross_val_poisson = function(form, df, corpus, tag, num_folds=5){
...
write_rds(llh_df, paste0(data_path, "en_zip_all_llh.rds"))
write_rds(coeff_count_df, paste0(data_path, "en_zip_all_coeffs_count.rds"))
write_rds(coeff_zero_df, paste0(data_path, "en_zip_all_coeffs_zero.rds"))
...
llh_df = read_rds(paste0(data_path, "en_zip_all_llh.rds"))
coeff_count_df = read_rds(paste0(data_path, "en_zip_all_coeffs_count.rds"))
coeff_zero_df = read_rds(paste0(data_path, "en_zip_all_coeffs_zero.rds"))
...
# Write the DLL to the same place that coeffs and LLHs are stored
write_rds(dll_df, paste0(data_path, "en_all_dll.rds"))
...
```{r}
dll_df = read_rds(paste0(data_path, "en_all_dll.rds"))
coeff_count_df = read_rds(paste0(data_path, "en_zip_all_coeffs_count.rds"))
coeff_zero_df = read_rds(paste0(data_path, "en_zip_all_coeffs_zero.rds"))
...
write.csv(plot_df, paste0(data_path, "en_plot_zip_df.csv"))
...
plot_df = read.csv(paste0(data_path, "en_plot_zip_df.csv")) %>%
  mutate(theory = case_when(
    tag == "NPMI" | tag == "E[NPMI]" ~ "Reanalysis",
    tag == "PPMI" | tag == "E[PPMI]" ~ "Reactivation"
  )) %>%
...
plot_df %>%
  ...
  ggplot(aes(x = tag, y=m, color = theory, fill=theory)) +
    ...
    geom_text(aes(label = sig, color = theory, y = case_when(corpus == "Provo"~ 0.12,
                                                             corpus == "UCL" ~ 0.023,
                                                             corpus == "Dundee" ~ 0.004)), size = 3, angle = 0) +
...
local_data_path = paste0(data_path, "combined_df_raw.csv")
...
coeff_agg %>%
  filter(tag == "PPMI", component == "count", corpus == "dundee")
...
coeff_agg_df %>%
  ...
  filter(coeff != "dist") %>%
  filter(window == "w0") %>%
  mutate(corpus = factor(corpus, levels = c("provo", "ucl", "dundee"), labels = c("Provo", "UCL", "Dundee"))) %>%
  ...
  ggplot(aes(x = plot_label, y = m, fill = plot_label)) +
    ...
    geom_errorbar(aes(ymin = lower, ymax = upper), color = "#8d8e8d", width = 0.05, position = position_dodge(width = 0.9)) +
  
    ylab("Coefficient Estimate (Scaled)") +
    xlab("") +
    #scale_fill_manual(values = c("#66b9c4","#668ac4", "#7166c4","#e5c539", "#e5a039", "#e56f39","#a6d854", "#63d854", "#14a04a")) +
    #scale_fill_manual(values = c("#66C2A5","#FC8D62","#8DA0CB","#E78AC3","#A6D854","#FFD92F","#E5C494","#B3B3B3", "#5f9e56")) +
  
  
    scale_fill_manual(values = c("#A6D854","#66C2A5","#5f9e56","#E78AC3","#FC8D62","#FFD92F","#8DA0CB","#B3B3B3", "#E5C494")) +
...
# Change commenting here to plot the "dist" subplot
ggsave("../images/exp_1/exp1_baseline_coeffs.pdf", device = "pdf", width = 8 , height = 4.5)
#ggsave("../images/exp_1/exp1_baseline_coeffs_dist.pdf", device = "pdf", width = 2.2 , height = 4) #This is for plotting the dist results
